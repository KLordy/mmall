...
public class CookieUtil {
    private static String COOKIE_DOMAIN="imooc.com";
    ...
    public static String readLoginToken(HttpServletRequest request){
        ...
        if(cks!=null){
            for(Cookie ck:cks){
                ...
                if(StringUtils.equals(ck.getName(),COOKIE_NAME)&&ck.getMaxAge()!=0){
    ...
    public static HttpServletResponse writeLoginToken(HttpServletResponse response, String token){
        ...
        ck.setPath("/");//代表设置在根目录
        ck.setHttpOnly(true);
        //单位是秒，如果这个maxage不设置的话，cookie就不会写入硬盘中，而是写在内存中，只在当前页面有效。
